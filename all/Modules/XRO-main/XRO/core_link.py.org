            ...
            for n in range(ncol):
                if n <= self.ac_order:
                    ...
                    for m in range(ncol):
                        if m <= self.ac_order:
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                _Cn_cos(X_active, t2d_x, n + m, self.omega, d)
                                + _Cn_cos(X_active, t2d_x, n - m, self.omega, d)
                            ) / 2
                        else:
                            mm = m - self.ac_order
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                _Cn_sin(X_active, t2d_x, n + mm, self.omega, d)
                                - _Cn_sin(X_active, t2d_x, n - mm, self.omega, d)
                            ) / 2
                else:
                    nn = n - self.ac_order
                    ...
                    for m in range(ncol):
                        if m <= self.ac_order:
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                _Cn_sin(X_active, t2d_x, nn + m, self.omega, d)
                                + _Cn_sin(X_active, t2d_x, nn - m, self.omega, d)
                            ) / 2
                        else:
                            mm = m - self.ac_order
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                -_Cn_cos(X_active, t2d_x, nn + mm, self.omega, d)
                                + _Cn_cos(X_active, t2d_x, nn - mm, self.omega, d)
                            ) / 2
