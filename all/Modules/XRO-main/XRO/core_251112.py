...
class XRO(object):
    ...
    def __compute__(self, X, Y, time=None, is_remove_nan=True, link_mask=None):
        """
        ...
        Lcoef = np.zeros((rank_y, rank_x, ncol))

        # If no link_mask provided, assume all active
        # print("link_mask received:", link_mask.shape, np.unique(link_mask))
        if link_mask is None:
            link_mask = np.ones((rank_y, rank_x), dtype=int)

        print("Y shape:", Y.shape)
        print("rank_y:", rank_y)
        for j in range(rank_y):
            # --- Select only active predictors for this Y_j ---
            active_idx = np.where(link_mask[j, :] == 1)[0]
            inactive_idx = np.where(link_mask[j, :] == 0)[0]
            print(f"Y={j}: active={active_idx}, inactive={inactive_idx}")
    
            X_active = X[active_idx, :]
            print(f"X_active shape: {X_active.shape}, total X shape: {X.shape}")
            r_active = len(active_idx)
            t2d_x_active = np.tile(tim, (r_active, 1))
    
            # --- Initialize G_j and C_j ---
            G_j = np.full((1, r_active * ncol), np.nan)
            C_j = np.full((r_active * ncol, r_active * ncol), np.nan)

            # Compute G_j and C_j as in standard XRO
            for n in range(ncol):
                if n <= self.ac_order:
                    d = self.taus[n]
                    G_j[:, r_active * n:r_active * (n + 1)] = _Gn_cos(Y[j:j + 1, :], X_active, t2d_y, n, self.omega, d)
                    for m in range(ncol):
                        if m <= self.ac_order:
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                _Cn_cos(X_active, t2d_x_active, n + m, self.omega, d)
                                + _Cn_cos(X_active, t2d_x_active, n - m, self.omega, d)
                            ) / 2
                        else:
                            mm = m - self.ac_order
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                _Cn_sin(X_active, t2d_x_active, n + mm, self.omega, d)
                                - _Cn_sin(X_active, t2d_x_active, n - mm, self.omega, d)
                            ) / 2
                else:
                    nn = n - self.ac_order
                    d = self.taus[nn]
                    G_j[:, r_active * n:r_active * (n + 1)] = _Gn_sin(Y[j:j + 1, :], X_active, t2d_y, nn, self.omega, d)
                    for m in range(ncol):
                        if m <= self.ac_order:
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                _Cn_sin(X_active, t2d_x_active, nn + m, self.omega, d)
                                + _Cn_sin(X_active, t2d_x_active, nn - m, self.omega, d)
                            ) / 2
                        else:
                            mm = m - self.ac_order
                            C_j[r_active * m:r_active * (m + 1), r_active * n:r_active * (n + 1)] = (
                                -_Cn_cos(X_active, t2d_x_active, nn + mm, self.omega, d)
                                + _Cn_cos(X_active, t2d_x_active, nn - mm, self.omega, d)
                            ) / 2

            # --- Solve for L_j with masked predictors ---
            L_j = _solve_L_with_zero(G_j, C_j)
            Lcoef_j = np.reshape(L_j, (r_active, ncol), order='F')

            # --- Insert into full-size Lcoef (inactive entries remain 0) ---
            for idx_a, var_idx in enumerate(active_idx):
                Lcoef[j, var_idx, :] = Lcoef_j[idx_a, :]
            
            # Y fit
            Y_fit = np.zeros(shape=Y.shape)
            Y_fit_j = np.zeros(ntim)
            for m in range(ncol):
                if m <= self.ac_order:
                    Y_fit_j += np.dot(Lcoef_j[:, m], X_active) * np.cos(m * self.omega * tim)
                else:
                    mm = m - self.ac_order
                    Y_fit_j += np.dot(Lcoef_j[:, m], X_active) * np.sin(mm * self.omega * tim)
            Y_fit[j, :] = Y_fit_j
    ...
    # ---------------------------------
    def fit(self, X, Y, time=None, is_remove_nan=True, link_mask=None):
        """
        ...
        fit_out = self.__compute__(
            X_np, Y_np, time=time, is_remove_nan=is_remove_nan, link_mask=link_mask)
    ...
    def fit_matrix(self, X, var_names=None, maskb=None, maskc=None,
                        maskNT=None, maskNH=None, time=None, link_mask=None):
        """
        ...
        for i in loop_index:
            ...
            var_res = self.fit(var_XN, var_YN, time=time, link_mask=link_mask)
