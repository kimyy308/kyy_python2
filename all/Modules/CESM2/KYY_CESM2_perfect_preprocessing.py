...
# Created 21-Nov-2024 by Yong-Yub Kim, kimyy308@pusan.ac.kr
...
class CESM2_config:
    ...
    def PFT_path_load(self, var):
        # PFT path check
        self.PFT_rootdir = '/mnt/lustre/proj/earth.system.predictability/PERFECT_TEST_timeseries/ASSM_EXP/archive'
        scenarios = r'\.(BHISTsmbb|BSSP370smbb)\.'
        command='ls ' + self.PFT_rootdir + '| grep BHISTsmbb | cut -d ''.'' -f 5-7'
        PFT_members_raw = subprocess.check_output(command, shell=True, text=True)
        self.PFT_members= [entry for entry in PFT_members_raw.split('\n') if entry]
        self.PFT_ensembles = [ens for ens in range(len(self.PFT_members))]
        PFT_file_list=[]
        PFT_ens_files=[]
        for member in self.PFT_members :
            PFT_files=[]    
            command='ls ' + self.PFT_rootdir + '/*' + member + '*/' + self.comp + '/proc/tseries/' + self.tfreq + '| grep \'\.' + self.var + '\.\''
            # print(command)
            PFT_files_raw = subprocess.check_output(command, shell=True, text=True)
            PFT_files= [entry for entry in PFT_files_raw.split('\n') if entry]
            PFT_files = sorted(PFT_files)
            ...
            for fname in PFT_files:
                ...
                if match1 and match2:
                    ...
                    if year1 >= self.year_s and year2 <= self.year_e:
                        ...
                        fpath=self.PFT_rootdir + '/' + 'b.e21.' + scenario + '.' + self.resol + '.' + member + '/' + self.comp + '/proc/tseries/' + self.tfreq + '/' + fname
                        # print(fpath)
                        PFT_filtered_files.append(fpath)
            PFT_ens_files.append(PFT_filtered_files)
        PFT_file_list.append(PFT_ens_files)
        self.PFT_file_list=PFT_file_list
