...
INITIAL_STATE_DAT="${AERA_HIST_DIR}/AERA_OmegaA_hist_nird_ann_OmegaA_co2_ems_2014.dat"

HIST_INPUT_CSV="/cluster/projects/nn2980k/yongyub/NORESM/NorESM2/input/AERA_hist/NorESM2-LM_esm-hist_r1i1p1f1_1850-2014_AERA_OmegaA_full_input.csv"

EMISSION_DIR="/cluster/projects/nn2980k/yongyub/NORESM/NorESM2/input/emissions/AERA/AERA_OmegaA_Betzy"

EMISSION_TEMPLATE_NC="${EMISSION_DIR}/emissions-cmip6_CO2_anthro_surface_AERA_OmegaA_hist_nird_2015-2019_201401-210112_fv_1.9x2.5.nc"
...
if [[ -z "${CURRENT_METRIC_CSV}" ]]; then
    CURRENT_METRIC_CSV="${MODEL_METRIC_DIR}/${AERAID}_model_OmegaA_${SEG_START}-${SEG_END}.csv"
fi
...
echo "[$(timestamp)] MODEL_METRIC_DIR       = ${MODEL_METRIC_DIR}"
...
python -u "${AERA_SCRIPT}" \
    ...
    --current-metric-csv "${CURRENT_METRIC_CSV}" \
