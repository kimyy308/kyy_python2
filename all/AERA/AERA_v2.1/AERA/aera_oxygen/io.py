...
def store_metadata(
        meta_file, oxygen_target_rel, oxygen_target_abs, year_x, model_start_year,
        s_oxygen_anth, s_total_emission, s_ff_emission, emission_curve):
    meta_file = Path(meta_file)
    timeseries_csv = Path(str(meta_file) + '.timeseries.csv')
    scalar_csv = Path(str(meta_file) + '.scalar.csv')

    timeseries_columns = [
        'total_emission', 'ff_emission', 'oxygen_anth', 'oxygen_anth_rel']
    scalar_columns = [
        'oxygen_target_abs', 'total_emission_budget', 'ff_emission_budget',
    ...
    df_scalar.loc[year_x, 'oxygen_target_abs'] = oxygen_target_abs
    ...
    for year in s_oxygen_anth.loc[:target_year_abs].index.values:
        df_timeseries.loc[
            (year_x, year), 'oxygen_anth'] = s_oxygen_anth.loc[year]
        df_timeseries.loc[
            (year_x, year), 'oxygen_anth_rel'] = (
                  s_oxygen_anth.loc[year] - oxygen_target_abs + oxygen_target_rel)
