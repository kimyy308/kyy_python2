...
# Same target setting used in AERA-Oxygen calculation
...
# Your Oxygen AERA run used y2 = 2014,
# so the calculated emissions are for 2015-2019.
stocktake_yr = 2014
nyr = 5
...
# AERA-Oxygen annual fossil-fuel emission output
AERA_CSV = (
    AERA_DIR / "output" / AERAid /
    f"{AERAid}_{stocktake_yr}.csv"
)

# Optional target-info file from the Oxygen calculation
TARGET_INFO_FILE = (
    AERA_DIR / "output" / AERAid /
    f"{AERAid}_target_info_{stocktake_yr}.csv"
)
...
print("Target info     :", TARGET_INFO_FILE)
...
with Dataset(OUT_FILE, "r+") as ds:
    ...
    ds.setncattr("AERA_branch", "Oxygen")
    ds.setncattr("AERA_variable", "O2var_0_200m_inventory")
    ...
    if target_info is not None:
        ...
        for key in [
            ...
            "reference_end",
            "reference_mean",
            "target_decrease_percent",
            "oxygen_target_abs",
            "stocktake_value",
        ]:
            if key in row.index:
                ds.setncattr(f"O2_{key}", row[key])

    ds.setncattr(
        "AERA_note",
        (
            "Template copied exactly; only CO2_flux values for "
            f"{target_start}-{target_end} were replaced using AERA-Oxygen "
