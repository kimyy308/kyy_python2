...
- extrapolated_runmean : Extrapolate a running mean at the beginning and end of the 
    time series.
- extrapolated_runmean_anth_oxygen : Calculate the extrapolated running
    mean of the observed/simulated oxygen. Calls the functions
    extrapolated_runmean and runmean.
- calculate_relative_target_oxygen : Calculate the relative target
    for oxygen saturation state [-].
...
def runmean(array, winlen):
    ...
    """
    print('--------------------------------')
    print('Executing AERA on branch AERA_2.1 (oxygen)')
    print('--------------------------------')
...
def extrapolated_runmean_anth_oxygen(year_x, model_start_year, s_oxygen, winlen): # YK
    """
    Calculate an extrapolated running mean of the simulated oxygen
    saturation state.
    ...
    Args:
        ...
        s_oxygen: Simulated oxygen timeseries
        winlen: window length of running mean for oxygen state fit

    Returns:
        Extrapolated running mean of the simulated oxygen. 
    """
    oxygen = np.array(s_oxygen.loc[model_start_year:year_x])
    
    return extrapolated_runmean(oxygen, winlen)


# Only needed for oxygen, because for temperature it is given as input
def calculate_relative_target_oxygen(  # YK
    oxygen_target_abs, s_oxygen, model_start_year):
    """
    Calculate the relative target for oxygen.
    Relative target oxygen, computed based on the method used for 
    computing the absolute temperature target for target type 2, see 
    calculate_absolute_target_temperature().
    We expect a negative relative oxygen target, because oxygen 
    is decreasing.
    
    Args:
        oxygen_target_abs (float): Absolute oxygen target
            (e.g. 2.75 or 1.0 []).
        s_oxygen: Simulated oxygen timeseries.
    ...
    Returns:
        oxygen_target_rel (float): relative oxygen target.
    """
    
    oxygen_target_rel = oxygen_target_abs - np.nanmean(s_oxygen.loc[model_start_year:1900].values)
    
    return oxygen_target_rel
...
def calculate_remaining_emission_budget( # YK
        oxygen_anth, total_emission, oxygen_target_abs, year_x,
        model_start_year, oxygen_abs_ts):
    
    """
    Calculate remaining emission budget.

    Args:
        oxygen_anth (array-like): Time series of anthropogenic oxygen
        (without any natural variablity).
        total_emission (array-like): Time series of total emissions.
        oxygen_target_abs (float): Absolute target oxygen.
        year_x (int): Current year in which the emissions for the next
            five years should be calculated.
        model_start_year (int): Year in which the historical simulation 
            (pre-cursor for the adaptive scenario simulation) was started.
        oxygen_abs_ts (array-like): Time series of measured/simulated
            oxygen (including natural variablity).

    Returns:
        reb (float): Remaining emission budget until the deoxygenation target 
            is reached in Pg C.
    """
    
    # Substract the reference period oxygen (1850-1900)
    doxygen_ref_yearx = (
        oxygen_anth.loc[year_x]) - oxygen_abs_ts.loc[model_start_year:1900].mean()
    print('Relative anthropogenic deoxygenation in Year X: ', doxygen_ref_yearx)
    ...
    slope = total_emission.loc[model_start_year:year_x -
                               1].sum() / doxygen_ref_yearx
    # Multiply TCRE with remaing allowable warming
    reb = (oxygen_target_abs - oxygen_anth.loc[year_x]) * slope
    print('REB: ', reb)
...
def get_adaptive_emissions( # YK
        oxygen_target_abs, year_x,
        model_start_year, df, meta_file, costum_anth_oxygen_func=None):
    """
    ...
    Args:
        oxygen_target_abs (float): oxygen target (e.g. 2.75 []).
        year_x (int): Current year in which the emissions for the next
            five years should be calculated.
        model_start_year (int): Year in which the historical
        ...
        df (pd.DataFrame): Pandas dataframe with years (int) as index
            ...
            - O2var:  Global or regional annual mean oxygen
    ...
    # We discard non-co2, because in the case of oxygen,
    # we want only ff + LUC in the computation of the TCRE.
    # Non-CO2 do not impact ocean acidification. (Marine's Ocean Acidification case)
    # Non-CO2 indirectly influence marine deoxygenation. (Yong-Yub's Marine Deoxygenation case)
    # emission_of_interest_cols = ['ff_emission', 'lu_emission'] # ML
    emission_of_interest_cols = ['non_co2_emission', 'ff_emission', 'lu_emission'] # YK
    ...
    # Calculate relative oxygen target
    oxygen_target_rel = calculate_relative_target_oxygen(oxygen_target_abs, df['O2var'], model_start_year)

    # Initialise the variable for anthropogenic oxygen time series until the time of the stocktake
    s_oxygen_anth = df['O2var'].loc[model_start_year:year_x].copy()

    # Extract anthropogenic oxygen time series
    # Second option does not exist for oxygen
    if costum_anth_oxygen_func is None:
        s_oxygen_anth.loc[:] = extrapolated_runmean_anth_oxygen(
            year_x,model_start_year, df['O2var'], winlen)
    else:
        s_oxygen_anth.loc[:] = costum_anth_oxygen_func(
            year_x, model_start_year, df['O2var'],
        )

    # Extract again the oxygen time series until 
    # the time of the stocktake, simulated/measured anthropogenic oxygen 
    # oxygen state will be needed later
    s_oxygen_abs = df['O2var'].loc[model_start_year:year_x].copy()

    # Calculate remaining emissions budget
    reb = calculate_remaining_emission_budget(
        s_oxygen_anth, s_emission_of_interest, oxygen_target_abs, year_x,
        model_start_year, s_oxygen_abs)
    ...
    # Calculate the slope of the emissions curve at year X-1 (In Yong-Yub's case, s_e_interest is same with s_total_emission
    ...
    # get_cheapest_curve actually does not need the argument 'oxygen_target_rel'
    ...
    #       'over next years:') #ML
    print('Carbon emissions [Pg C] (fossil-fuel CO2 + land use + non-CO2 ) ' 
          'over next years:') #YK
    ...
    # We need to subtract LUC and nonCO2 emissions in the case of MD
    # s_ff_emission = (s_emission_of_interest - df['lu_emission']) # ML
    s_ff_emission = (s_emission_of_interest - df['lu_emission'] - df['non_co2_emission']) # YK
    s_ff_emission.name = 'ff_emission'
    
    # Store data to metafile for debug and post-analysis
    io.store_metadata(
        meta_file, oxygen_target_rel, oxygen_target_abs, year_x,
        model_start_year, s_oxygen_anth, s_emission_of_interest, s_ff_emission, ec)
